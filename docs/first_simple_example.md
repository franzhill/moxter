...
## The YAML Definition
Place a file named  `moxtures.yaml` in your test resources (e.g., `src/test/resources/moxter/simple/`). This file describes a HTTP call to your API backend.
...
class PetIntegrationTest {
    ...
    void testShampooOnPet() {
        ...
        // (The return status code has automatically been verified asserted by Moxter.)
...
At this point with a simple test like this one, what we have saved ourselves is essentialy boilerplate code, while bringing clarity, separation of concern and reusability.

### Boilerplate and mixing concerns

Had we not leveraged Moxter, we would have had to code our test something like so, heavily mixing 
...
By extracting the configuration details of the REST request into a configuration, we have made it 
possible to call that request again and again, possibly introducing slight variations too by 
shadowing some of its configurations at call time.
